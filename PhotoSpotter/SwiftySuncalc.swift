...
public class SwiftySuncalc {
    ...
    public func getMoonPosition(date: Date, lat: Double, lng: Double) -> Dictionary<String, Double> {
        ...
        let c: Dictionary<String, Double> = moonCoords(d: d)
    ...
    public func getMoonIllumination(date: Date) -> Dictionary<String, Double> {
        ...
        let m: Dictionary<String, Double> = moonCoords(d: d)
        // Distance from Earth to Sun in km
        let sdist: Double = 149598000.0
        
        let phi   : Double = acos(sin(s["dec"]!) * sin(m["dec"]!) + cos(s["dec"]!) * cos(m["dec"]!) * cos(s["ra"]! - m["ra"]!))
        let inc   : Double = atan2(sdist * sin(phi), m["dist"]! - sdist * cos(phi))
        let angle : Double = atan2(cos(s["dec"]!) * sin(s["ra"]! - m["ra"]!),  sin(s["dec"]!) * cos(m["dec"]!) - cos(s["dec"]!) * sin(m["dec"]!) * cos(s["ra"]! - m["ra"]!))
    ...
    public func getMoonTimes(date: Date, lat: Double, lng: Double) -> Dictionary<String, Date?> {
        ...
        for i in stride(from: 1, to: 24, by: 2) {
            h1    = getMoonPosition(date: hoursLater(date: t, h: Double(i)), lat: lat, lng: lng)["altitude"]! - hc
            h2    = getMoonPosition(date: hoursLater(date: t, h: Double(i + 1)), lat: lat, lng: lng)["altitude"]! - hc
            a     = (h0 + h2) / 2.0 - h1
            b     = (h2 - h0) / 2.0
            xe    = -b / (2.0 * a)
            ye    = (a * xe + b) * xe + h1
